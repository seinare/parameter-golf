...
class Hyperparameters:
    ...
    muon_bucketed = bool(int(os.environ.get("MUON_BUCKETED", "0")))
...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        super().__init__(
            params,
            dict(lr=lr, momentum=momentum, backend_steps=backend_steps, nesterov=nesterov, bucketed=bucketed),
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            bucketed = group["bucketed"] and world_size == 1
            ...
            if bucketed:
                offsets = {}
                curr = 0
                buckets: dict[tuple[torch.Size, torch.device, torch.dtype], list[torch.nn.Parameter]] = {}
                for p in params:
                    offsets[p] = curr
                    curr += p.numel()
                    if p.grad is not None:
                        buckets.setdefault((p.shape, p.device, p.dtype), []).append(p)
                for bucket_params in buckets.values():
                    grads = []
                    buffers = []
                    for p in bucket_params:
                        ...
                        buffers.append(buf)
                        grads.append(p.grad.add(buf, alpha=momentum) if nesterov else p.grad)
                    g = torch.stack(grads, dim=0)
                    g = zeropower_via_newtonschulz5(g, steps=backend_steps)
                    g *= max(1, g.size(-2) / g.size(-1)) ** 0.5
                    for p, update in zip(bucket_params, g, strict=True):
                        start = offsets[p]
...
def main() -> None:
    ...
    log0(
        ...
        f"muon_bucketed:{int(args.muon_bucketed)}"
    )
    log0(
